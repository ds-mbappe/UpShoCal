...
const Calendar = ({ navigation }) => {
    return (
        ...
        renderItem={(item, firstItemInDay) => {
            return (
                <View style={{flex: 1, justifyContent: 'center', alignItems: 'flex-start', borderColor: 'red', borderWidth: 2}}>
        ...
        // Specify how each date should be rendered. day can be undefined if the item is not first in that day
            ...
            const dayExpression = day.getDay();
            
            switch(dayExpression) {
                case 0:
                    return (<Text>Mon</Text>);
                case 1:
                    return (<Text>Tue</Text>);
                case 2:
                    return (<Text>Wed</Text>);
                case 3:
                    return (<Text>Thu</Text>);
                case 4:
            ...
            return (
                <View style={{flexDirection: 'column', width: 40, height: 150, margin: 5, justifyContent: 'center', alignItems: 'center', borderColor: 'blue', borderWidth: 2}}>
                    <Text style={{fontSize: 50, color: 'gray'}}>{day.getDate()}</Text>
                    <Text style={{fontSize: 25, color: 'gray'}}>{day.getDay()}</Text>
                </View>
            );
        }}
