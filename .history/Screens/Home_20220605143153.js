...
const Home = () => {
    ...
    useEffect(() => {
        const getShoesData = async () => {
            try {
                await axios.get("https://stockx.com/api/browse?productCategory=sneakers&sort=release_date&releaseTime=gte-1654384198&order=ASC&country=FR", {
                    headers: {
                        'Access-Control-Allow-Origin': 'https://stockx.com'
                    }
                })
                    .then(response => {
                        setShoesData(response.data);
                        console.log(shoesData.Product);
                });
            } catch (error) {
                console.log("Nothing");
            }
